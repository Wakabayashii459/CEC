...
final_string_arr = np.zeros((len(thresholds), 501), int)
...
for thres in thresholds:
    ...
    prices = product_data.values[2016:2520, 4]  # GOING FOR SMALL NOW!! DO NOT FORGET 10 YEARS
    #threshold = 0.025
    p_high = prices[0]
    p_low = prices[0]
    index_dc = [0, 0]
    ...
    strat_list_1 = strat_list_3_buy + strat_list_3_sell
    ...
    final_string_arr[count][buy_position] = 1; final_string_arr[count][sell_position] = 2
...
results_arr=results_arr.reshape(4,10)
debug=0
...
with open('IndividualResults/' + stock[0] + '/TestMetrics.txt', 'w') as f:
